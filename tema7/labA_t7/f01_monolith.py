...
def register_model(
    ...
    logger = get_run_logger()
    model_path = Path(tempfile.gettempdir()) / "titanic_knn_monolith.joblib"
    joblib.dump(model, model_path)

    artifact = wandb.Artifact(
        name=config["model_artifact_name"],
        type=config["model_artifact_type"],
        description="Serialized Titanic KNN model from the monolithic Prefect flow.",
    )
...
@flow(name="Titanic Monolith - Full pipeline")
def full_pipeline() -> None:
    config = load_config("f01_monolith_config")
